...
async function inspectExistingBundle(directory) {
  ...
  try {
    ...
    const log = relativeFiles.includes("log.md") ? await readFile(logPath, "utf8") : void 0;
    return { root, markdownFiles: relativeFiles, conceptPaths, conceptContents, ...log === void 0 ? {} : { log } };
  } catch (error) {
...
async function renderBundle(plan, outputDirectory, options = {}) {
  ...
  if (options.includeLog !== false) {
    ...
    await writeFile(path2.join(root, "log.md"), log, "utf8");
    files.push("log.md");
  }
...
async function readConcept(root, absolute) {
  const file = toPosix3(path7.relative(root, absolute));
...
import { chmod, mkdir as mkdir2, readFile as readFile5, writeFile as writeFile2 } from "fs/promises";
import os from "os";
...
var okfEnvDirectory = path8.join(os.homedir(), ".okf");
var okfEnvPath = path8.join(okfEnvDirectory, ".env");
var managedEnvKeys = [
...
async function saveOkfEnv(updates, filePath = okfEnvPath, environment = process.env) {
  ...
  await mkdir2(path8.dirname(filePath), { recursive: true, mode: 448 });
  await chmod(path8.dirname(filePath), 448);
...
async function readEnvFile(filePath) {
  try {
    return parseEnv(await readFile5(filePath, "utf8"));
  } catch (error) {
    if (error.code === "ENOENT") return {};
    throw error;
  }
...
import { access, readFile as readFile6, writeFile as writeFile3 } from "fs/promises";
...
async function findProjectConfig(startDirectory = process.cwd()) {
  let directory = path9.resolve(startDirectory);
  while (true) {
    const candidate = path9.join(directory, DEFAULT_PROJECT_CONFIG);
    try {
...
async function loadProjectConfig(filePath) {
  const resolvedPath = filePath ? path9.resolve(filePath) : await findProjectConfig();
  if (!resolvedPath) return { config: projectConfigSchema.parse({}) };
  let document;
  try {
    document = parse(await readFile6(resolvedPath, "utf8"));
  } catch (error) {
  ...
  if (!result.success) throw new Error(`Invalid ${path9.basename(resolvedPath)}: ${result.error.issues.map((issue) => `${issue.path.join(".") || "configuration"}: ${issue.message}`).join("; ")}`);
...
async function createProjectConfig(filePath = DEFAULT_PROJECT_CONFIG, force = false) {
  ...
  if (!force) {
    try {
      ...
      throw new Error(`${path9.basename(resolvedPath)} already exists. Use --force to replace it.`);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
  }
...
async function buildViewerData(directory) {
  ...
  const conceptFiles = files.filter((file) => !["index.md", "log.md"].includes(path10.basename(file)));
  ...
  const records = await Promise.all(conceptFiles.map(async (file) => {
    const id = toPosix4(path10.relative(root, file));
    const raw = await readFile7(file, "utf8");
    const parsed = matter5(raw);
    const title = stringValue(parsed.data.title) || humanize2(path10.basename(id, ".md"));
    return {
      concept: {
        id,
        title,
  ...
  for (const { concept, raw } of records) {
    for (const match of raw.matchAll(/\[[^\]]*\]\(([^)]+)\)/g)) {
      ...
      const target = href.startsWith("/") ? path10.posix.normalize(href.slice(1)) : path10.posix.normalize(path10.posix.join(path10.posix.dirname(concept.id), href));
...
async function readBundleTitle(root) {
  try {
    const content = await readFile7(path10.join(root, "index.md"), "utf8");
    const heading = matter5(content).content.match(/^#\s+(.+)$/m)?.[1]?.trim();
    return heading || humanize2(path10.basename(root));
  } catch {
    return humanize2(path10.basename(root));
  }
}
